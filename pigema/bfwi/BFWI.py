...
class Bayesian_Full_Waveform_Inversion:
    ...
    #===========================================
    # Gaussian Prior Negative Log-Likelihood
    #===========================================
    ...
    def Linear_Interpolation(
        ...
        if is_sorted and y_finite_all:
            x_src_sig = (
                int(x_src.size),
                to_scalar_float(x_src[0]),
                to_scalar_float(x_src[-1]),
                to_scalar_float(x_src[1] - x_src[0]) if x_src.size > 1 else 0.0,
            )
            xq_sig = (
                int(xq.size),
                to_scalar_float(xq[0]),
                to_scalar_float(xq[-1]),
                to_scalar_float(xq[1] - xq[0]) if xq.size > 1 else 0.0,
            )
            cache_key = (x_src_sig, xq_sig, str(dtype), to_scalar_float(eps))
